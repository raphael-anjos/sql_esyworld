#Data 21/11/2016
#Retorna todos os fornecedores cadastrados que não tiveram
#títulos gerados em contas a pagar dentro do período informado 
#entre dtInicial e dtFinal
SET @dtInicial = '2016-01-01'; 
SET @dtFinal = '2016-11-21';
SELECT cod_fornecedor, razao_social, nome_fantasia
FROM cd_fornecedor
WHERE cod_fornecedor NOT IN (
SELECT DISTINCT(cod_fornecedor)
FROM fn_titulo_pagar
WHERE dt_emissao >= @dtInicial AND 
		dt_emissao <= @dtFinal);


#Data 21/11/2016
#inativa no cadastro de fornecedores todos os fornecedores que não tiveram
#títulos gerados em contas a pagar dentro do período informado 
#entre dtInicial e dtFinal
SET @dtInicial = '2016-01-01';
...
UPDATE cepbai
SET bairro = REPLACE(bairro,'VARZENA','VZA');SET @dtFinal = '2016-11-21';
UPDATE cd_fornecedor SET situacao = 0
WHERE cod_fornecedor NOT IN (
SELECT DISTINCT(cod_fornecedor)
FROM fn_titulo_pagar
WHERE dt_emissao >= @dtInicial AND 
